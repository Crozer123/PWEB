    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
        ...
        <div class="space-y-6">
            <div class="relative aspect-square bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 group">
                ...
                @if($item->image)
                    {{-- PERBAIKAN 1: Gambar Utama --}}
                    <img src="{{ \Illuminate\Support\Str::startsWith($item->image, 'http') ? $item->image : asset('storage/' . $item->image) }}" 
                         alt="{{ $item->name }}" 
                         class="w-full h-full object-cover p-8 transition duration-500 group-hover:scale-105">
                @else
